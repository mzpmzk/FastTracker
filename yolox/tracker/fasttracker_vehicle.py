...
reset_velocity_offset_occ = 10
reset_pos_offset_occ = 3
enlarge_bbox_occ = 1.2
dampen_motion_occ = 0.80
active_occ_to_lost_thresh = 50
...
class Fasttracker(object):
    ...
    def update(self, output_results, img_info, img_size):
        ...
        for it in u_track:
            ...
            if not track.is_occluded and track.state == TrackState.Tracked:
                for other in activated_starcks:
                    ...
                    if is_occluded_by(track.tlbr, other.tlbr):
                        ...
                        # Reset velocity
                        if len(track.mean_history) >= reset_velocity_offset_occ:
                            old_mean = track.mean_history[-reset_velocity_offset_occ]
                            track.mean[4:8] = old_mean[4:8]

                        # Reset position
                        if len(track.mean_history) >= reset_pos_offset_occ:
                            old_mean = track.mean_history[-reset_pos_offset_occ]
                            track.mean[0:4] = old_mean[0:4]

                        # Enlarge once
                        if track.occluded_len == 1:
                            track.mean[3] *= enlarge_bbox_occ  # increase height
                            # track.mean[2] = track.mean[2] / track.mean[3]  # adjust aspect ratio

                        # Dampen motion
                        track.mean[4:8] *= dampen_motion_occ
                        break
            ...
            if track.state != TrackState.Lost:
                if track.not_matched > 2 and (
                    not track.is_occluded or track.occluded_len > active_occ_to_lost_thresh
                ):
                    track.mark_lost()
                    lost_stracks.append(track)
        ...
        output_stracks = [track for track in self.tracked_stracks if (track.is_activated and not track.is_occluded)]
